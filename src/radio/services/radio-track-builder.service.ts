...
import { BuildOptions, BuildStrategy, TrackMeta } from '../radio.types'
import { KafkaService } from '../../kafka/kafka.service'
import { MusicRepository } from '../repositories/music.repository'
import { KafkaTopic } from '../../kafka/kafka.constant'

@Injectable()
export class RadioTrackBuilderService {
  private readonly logger = new Logger(RadioTrackBuilderService.name)

  constructor(
    private readonly musicRepo: MusicRepository,
    private readonly playlistService: RadioTrackService,
  ...
  async build(opts: BuildOptions = {}): Promise<number> {
    ...
    await this.kafka.send(KafkaTopic.playlist_build, [
      {
        value: {
          strategy,
          count: tracks.length,
  ...
  async addTracksToActive(trackIds: string[]): Promise<number> {
    ...
    if (!tracks.length) {
      this.logger.warn(`No active tracks found for IDs: ${trackIds.join(', ')}`)
      return 0
    }
    ...
    await this.kafka.send(KafkaTopic.track_added_manually, [
      {
        value: {
          count: tracks.length,
